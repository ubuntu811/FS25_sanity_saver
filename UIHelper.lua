...
---@class UIHelper
-- Guarded, not in the original file: this is a shared community utility
-- other mods vendor too (confirmed - FS25_AdditionalContracts ships this
-- exact file). Without the guard, whichever mod's copy loads last would
-- silently stomp the other's UIHelper global. Since both copies are
-- functionally identical, whichever loads first "wins" and the rest just
-- skip redefining it - harmless either way.
if UIHelper == nil then
UIHelper = {}

---Creates a new section with the given title
---@param gameSettingsPage table @The general settings page of the base game
...
local function createElement(gameSettingsPage, template, id, i18nTextId, target, callbackFunc)
	...
	textElement:setText(g_i18n:getText(i18nTextId .. "_title"))
	-- Change the tooltip
	local toolTip = elementOption.elements[1]
	toolTip:setText(g_i18n:getText(i18nTextId .. "_info"))
...
---@param i18nTextId            string      @The key in the internationalization XML (must be two keys with a _title and _info suffix)
...
---@return                      table       @The created object
function UIHelper.createChoiceElement(gameSettingsPage, id, i18nTextId, i18nValueMap, target, callbackFunc, nillable)
...
---For every control name, a <prefix>_<name>_info and _title text must exist in the l10n files.
---The _title text will be the title of the setting, the _info" text will be its tool tip.
...
function UIHelper.createControlsDynamically(settingsPage, sectionTitle, owningTable, controlProperties, prefix)
	...
	for _, controlProps in ipairs(controlProperties) do
		...
		uiControl.typName = controlProps.typName;
